...
def create_user(
    ...
    if not re.match(r'^[0-9]{2}\.[0-9]{2}\.[0-9]{4}$', geburtsdatum):
        raise HTTPException(status_code=400, detail="Geburtsdatum muss im Format DD.MM.YYYY eingegeben werden (z.B. 12.03.1990)")
    
    # Validate phone format
    if not re.match(r'^\+41[0-9]{9}$', telefon):
        raise HTTPException(status_code=400, detail="Telefonnummer muss im Format +41791234567 eingegeben werden")
    
    # Validate postleitzahl (4 digits)
    if not re.match(r'^[0-9]{4}$', postleitzahl):
        raise HTTPException(status_code=400, detail="Postleitzahl muss 4-stellig sein")
